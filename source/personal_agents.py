...
# Import the tools
from source.weather_tools import WeatherTools
from source.calendar_tools import CalendarTools
from source.email_tools import GmailTools
from source.search_tools import SearchTools
...
def _get_current_datetime_str() -> str:
    """Current date/time in the configured timezone, formatted for direct embedding into
    system prompts -- agents read this instead of calling a get_current_datetime tool."""
    tz = pytz.timezone(config.DEFAULT_TIMEZONE)
    return datetime.now(tz).strftime("%A, %Y-%m-%d %H:%M %Z")


def create_model_client() -> OpenAIChatCompletionClient:
    """Create and configure the Groq model client via its OpenAI-compatible endpoint."""
...
async def create_email_agent(model_client: OpenAIChatCompletionClient) -> AssistantAgent:
    ...
    return AssistantAgent(
        ...
        tools=email_tools,
        system_message=email_system_prompt.format(current_datetime=_get_current_datetime_str(), timezone=config.DEFAULT_TIMEZONE),
...
async def create_weather_agent(model_client: OpenAIChatCompletionClient) -> AssistantAgent:
    ...
    return AssistantAgent(
        ...
        model_client=model_client,
        tools=weather_tools,
        system_message=weather_system_prompt.format(current_datetime=_get_current_datetime_str(), timezone=config.DEFAULT_TIMEZONE),
        reflect_on_tool_use=False,
    )
...
async def create_calendar_agent(model_client: OpenAIChatCompletionClient) -> AssistantAgent:
    ...
    return AssistantAgent(
        ...
        system_message=calendar_system_prompt.format(current_datetime=_get_current_datetime_str(), timezone=config.DEFAULT_TIMEZONE),
        reflect_on_tool_use=False,
    )
...
class PersonalAssistantOrchestrator:
    ...
    def __init__(self):
        ...
        # The selector's first agent pick has proven reliable in testing; letting the chat
        # continue past that (hoping for a TERMINATE) let unrelated agents hijack a correct
        # answer instead. Cap at exactly one agent turn: task message + that agent's reply.
        self.termination = TextMentionTermination("TERMINATE") | MaxMessageTermination(2)
    ...
    async def process_request(self, user_input: str) -> str:
        ...
        try:
            ...
            # Create team with all agents. SelectorGroupChat makes a single LLM call to
            # pick the next agent based on its `description`, instead of MagenticOneGroupChat's
            # heavier plan/re-plan/reflect loop -- far fewer LLM calls per simple request.
            ...
            # Final response: use last message's content, stripped of the TERMINATE marker
